...
DRAW_DIST = 1000 
MAP_SIZE = 200
PROXIMITY_RANGE = 1.5  # <--- Change this for precision (smaller = harder)
...
class Player:
    def __init__(self, x, z, color, controls, laser_color, serial_port=None):
        ...
        self.controls = controls # [Forward, Fire, Left, Right]
        self.speed = 0.0
        self.pulses = 0 
        self.rockets = []
...
def draw_custom_rider(screen, bx, by, sprite_h, target, obs):
    ...
    base_w = 16 + int(28 * aspect); body_h = sprite_h // 2.5
...
def draw_arena(screen, obs, target, grid, x_offset, clouds, cur_w, cur_h):
    ...
    if abs(sun_rel) < FOV:
        pygame.draw.circle(screen, (255, 255, 210), (int(x_offset + (sun_rel/FOV + 0.5)*view_w), int(cur_h//2 - (SUN_ELEVATION*(cur_h//2.5)))), 40)

    for c in clouds:
        dx, dz = c.world_x - obs.x, c.world_z - obs.z
        c_dist = math.sqrt(dx*dx + dz*dz); c_ang = math.atan2(dz, dx) - obs.angle
    ...
    for i in range(NUM_RAYS):
        ...
        for d in range(1, DRAW_DIST):
            ...
            if tile >= 3:
                ...
                wall_h = (cur_h / (dist + 0.001)) * 2.0; fog = max(0, min(1, 1 - (dist / (DRAW_DIST * 0.18))))
    ...
    if abs(t_ang) < FOV:
        ...
        pygame.draw.line(screen, target.laser_color, (tx_s, 0), (tx_s, cur_h // 2), 2)
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_f:
                ...
                if fullscreen:
                    ...
                    # Explicitly snap back to base resolution
                    screen = pygame.display.set_mode((res_w, res_h), pygame.RESIZABLE)
        ...
        screen.fill((0, 0, 0))
        draw_arena(screen, p1, p2, grid, 0, clouds, cw, ch)
        draw_arena(screen, p2, p1, grid, cw//2, clouds, cw, ch)
        
        # Center Line & Scoreboard
        pygame.draw.line(screen, (255, 255, 255), (cw//2, 0), (cw//2, ch), 4)
        score_surf = font.render(f"{int(p1.score)}   |   {int(p2.score)}", True, (255, 255, 255))
        hud_w = score_surf.get_width() + 20
        pygame.draw.rect(screen, (0, 0, 0), (cw//2 - hud_w//2, 10, hud_w, 45))
