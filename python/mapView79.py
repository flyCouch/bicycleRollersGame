...
class Rocket:
    def __init__(self, x, z, angle, speed, color):
        self.x, self.z = x, z
        self.angle = angle
        self.speed = speed + 0.96 # 20% slower base
        self.color = color
...
def draw_custom_rider(screen, bx, by, sprite_h, target, obs):
    ...
    r_off, l_off = math.cos(rel_ang + 1.57) * spread, math.cos(rel_ang - 1.57) * spread
...
def draw_arena(screen, obs, target, grid, x_offset, clouds, cur_w, cur_h):
    ...
    for p in [obs, target]:
        for r in p.rockets:
            ...
            if abs(rang) < FOV/2:
                pygame.draw.circle(screen, r.color, (int(x_offset + (rang/FOV + 0.5)*view_w), horizon), max(4, int(cur_h/(rdist+0.001))//4))
