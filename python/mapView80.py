...
class Rocket:
    def __init__(self, x, z, angle, speed, color):
        self.x, self.z = x, z
        self.angle = angle
        self.speed = speed + 0.96 # Corrected 20% slow speed
        self.color = color
...
class Player:
    ...
    def serial_thread(self):
        try:
            # Strictly private serial instance
            ser = serial.Serial(self.port_name, BAUD_RATE, timeout=0.01)
            print(f"SUCCESS: {self.name} connected to {self.port_name}")
            while True:
                line = ser.readline().decode('utf-8').strip()
                if line == "1":
                    self.pulses += 1
        except:
            print(f"INFO: {self.name} port {self.port_name} not found. Keyboard only.")
    ...
    def update(self, keys, grid, sounds, opponent):
        ...
        # Speed is strictly instance-bound
...
def draw_arena(screen, obs, target, grid, x_offset, clouds, cur_w, cur_h):
    ...
    for c in clouds:
        ...
        c_dist = math.sqrt(dx*dx + dz*dz) # Typo fixed here (dz*dz)
...
def main():
    ...
    with open("mega_arena.json", "r") as f:
    ...
    while True:
        ...
        p1.update(keys, grid, sounds, p2)
        p2.update(keys, grid, sounds, p1)
