...
def draw_custom_rider(screen, bx, by, sprite_h, target, obs):
    ...
    pygame.draw.ellipse(screen, (5, 20, 5), (bx - base_w//2 + sh_dx, by + body_h - 2, base_w, 8))
    
    # 2. Body
    pygame.draw.ellipse(screen, target.color, (bx - base_w//2, by, base_w, body_h))
    
    # 3. Hands (Anatomical: Green=Right, Blue=Left)
    h_size = max(4, body_h // 6)
    hand_spread = (base_w // 2) + 2
    
    # Right Hand (Green)
    r_h_offset = math.cos(rel_ang + math.pi/2) * hand_spread
    pygame.draw.circle(screen, (50, 255, 120), (int(bx + r_h_offset), int(by + body_h//2)), int(h_size))
    
    # Left Hand (Blue)
    l_h_offset = math.cos(rel_ang - math.pi/2) * hand_spread
    pygame.draw.circle(screen, (50, 120, 255), (int(bx + l_h_offset), int(by + body_h//2)), int(h_size))
...
def draw_arena(screen, obs, target, grid, x_offset, clouds, cur_w, cur_h):
    ...
    while sun_rel < -math.pi: sun_rel += 2*math.pi
    if abs(sun_rel) < FOV:
        ssx = x_offset + (sun_rel / FOV + 0.5) * view_w
    ...
    # 5. Laser & Rider (Beacon logic draw last)
