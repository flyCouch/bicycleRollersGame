...
def draw_custom_rider(screen, bx, by, sprite_h, target, obs):
    ...
    base_w = max(12, int(14 + 36 * aspect))
    body_h = max(12, sprite_h // 2.5)
...
def draw_arena(screen, obs, target, grid, x_offset, clouds, cur_w, cur_h):
    ...
    for c in clouds:
        ...
        if abs(c_ang) < FOV * 1.5: # Buffered FOV for clouds
    ...
    # Use a buffered FOV (1.5x) so player doesn't disappear at screen edge
    if abs(t_ang) < FOV * 0.75: 
        tx_s = x_offset + (t_ang / FOV + 0.5) * view_w
        
        # Always draw laser beacon for visibility
        pygame.draw.line(screen, target.laser_color, (int(tx_s), 0), (int(tx_s), horizon), 2)
        
        # Determine ray index for Z-Buffer check
        idx = int((t_ang / FOV + 0.5) * NUM_RAYS)
        idx = max(0, min(NUM_RAYS - 1, idx))
        
        # Overdraw logic: prioritize player over walls
        if t_dist < z_buffer[idx] * 1.1: 
